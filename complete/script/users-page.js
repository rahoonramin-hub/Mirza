...
import { onAuth, loadData, saveData, render_top_box, escapeHtml, go_home, calculateTotals, go_settings } from './common.js';
...
export async function renderUsers() {
    ...
    users.forEach(user => {
        ...
        let lastTap = 0;
        userEl.addEventListener('touchend', (e) => {
            const now = Date.now();
            if (now - lastTap < 300) {
                clearTimeout(tapTimeout);
                const touch = e.changedTouches[0];
                showUserContextMenu(touch.clientX, touch.clientY, user, userEl);
            } else {
...
function prof_input() {
    ...
    profInput.onchange = () => {
        ...
        reader.onload = (e) => prof.src = e.target.result;
...
async function add_user() {
    ...
    let prof = "img.JPG";
    if (file) {
        prof = await new Promise((resolve) => {
            const reader = new FileReader();
    ...
    const newUser = createUser(name, description, phone_num, telegram_id, prof, book_address, user_id);
...
function showEditUserModal(user) {
    ...
    saveBtn.onclick = async () => {
        ...
        if (profInput.files.length > 0) {
            const file = profInput.files[0];
            targetUser.prof = await new Promise((resolve) => {
                const reader = new FileReader();
                reader.onload = (e) => resolve(e.target.result);
                reader.readAsDataURL(file);
            });
...
onAuth(async (isLoggedIn) => {
    if (isLoggedIn) {
        ...
        // اتصال دکمه‌ها (اگر از onclick در HTML استفاده نکردید)
...
// دیگر نیازی به DOMContentLoaded جداگانه نیست
// چون onAuth خودش صبر می‌کند تا وضعیت احراز هویت مشخص شود.